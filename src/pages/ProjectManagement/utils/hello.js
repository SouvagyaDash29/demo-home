...
import { addDays, format, isSameDay, subDays, parse } from 'date-fns';
import {
  buildTasksByDate,
  formatToDDMMYYYY,
...
const ProjectManagementTimesheetEmployee = () => {
  ...
  const getAllLogDates = (project) => {
    if (!project?.day_logs) return [];
    return Object.keys(project.day_logs)
      .map(parseLogDate)
      .filter(Boolean);
  };

  const getPlannedDateRange = (project) => {
  ...
  const hasPendingCheckoutFromYesterday = (project, yesterdayDate) => {
    if (!project?.pendingCheckoutDate) return false;
    const pendingDate = parseLogDate(project.pendingCheckoutDate);
    if (!pendingDate) return false;
    return isSameDay(pendingDate, yesterdayDate);
  };
  ...
  const finalProjects = filteredProjects.filter((project) => {
    const logDates = getAllLogDates(project);
    const plannedRange = getPlannedDateRange(project);
    const hasPendingCheckout = project?.hasPendingCheckout;
    const pendingFromYesterday = hasPendingCheckoutFromYesterday(project, yesterday);

    if (dayFilter === "past7") {
      return logDates.some((date) => {
        if (!date) return false;
        return date >= pastWindowStart && date <= yesterday;
      });
    }

    if (dayFilter === "today") {
      const hasTodayLog = logDates.some((date) => date && isSameDay(date, today));
      const plannedToday =
        plannedRange &&
        today >= plannedRange.start &&
        today <= plannedRange.end;
      return hasTodayLog || plannedToday || (hasPendingCheckout && pendingFromYesterday);
    }

    if (dayFilter === "next7") {
      if (!plannedRange) return false;
      const upcomingWindow = { start: tomorrow, end: upcomingWindowEnd };
      return rangesOverlap(plannedRange, upcomingWindow);
    }
    ...
    if (dayFilter === "custom") {
      ...
      const logsInRange = logDates.some((date) => date && date >= from && date <= to);
      const plannedMatches = rangesOverlap(plannedRange, { start: from, end: to });
      return logsInRange || plannedMatches;
    }
  ...
  const openFormModal = (project, modalContext = {}) => {
    setSelectdeProject({
      ...project,
      modalContext
    })
  ...
  const handleOpenContinueModal = (project, remarkRequired) => {
  ...
  const handleOpenCompleteModal = (project) => {
    openFormModal(project, {
      type: "complete",
      forceMode: "UPDATE"
    })
